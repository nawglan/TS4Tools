...
/// </remarks>
internal class WorldColorTimelineResource : IWorldColorTimelineResource, IDisposable
{
    ...
    {
        get => throw new NotSupportedException($"WorldColorTimelineResource does not support indexed access. Index: {index}");
        set => throw new NotSupportedException($"WorldColorTimelineResource does not support indexed access. Index: {index}");
    }

    public TypedValue this[string field]
    {
        get => throw new NotSupportedException($"WorldColorTimelineResource does not support field access. Field: {field}");
        set => throw new NotSupportedException($"WorldColorTimelineResource does not support field access. Field: {field}");
    }
    ...
    {
        // Create a new writable MemoryStream and copy stream data to it
        ...
        using var reader = new BinaryReader(_stream, global::System.Text.Encoding.UTF8, true);
    ...
    {
        // Create a new writable stream for the updated data
        using var tempStream = new MemoryStream();
        Save(tempStream);

        // Replace the old stream with the new one
        _stream?.Dispose();
        _stream = new MemoryStream(tempStream.ToArray());
    }
    ...
    private IColorTimeline LoadColorTimeline(BinaryReader reader)
    {
        ...
        // Version 14+ features
        if (timeline.Version >= SupportedVersion14)
        {
    ...
    /// <summary>
    /// Saves the resource data to a stream (explicit interface implementation).
    /// </summary>
    /// <param name="stream">The stream to save to.</param>
    void IWorldColorTimelineResource.Save(Stream stream) => Save(stream);
...
internal class ColorTimelineData : IColorTimelineData
{
    ...
    public IColorData GetColorAtTime(float time)
    {
        ...
        {
            return new ColorData { Time = time, Alpha = 0.0f };
        }
