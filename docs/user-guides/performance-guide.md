...
```csharp
// âœ… For large resources - use streams
...
// âœ… For small resources - byte arrays are fine
...
// âŒ Avoid - Loading large resources into memory
...
// âŒ Avoid - Blocking on async operations
var package = packageFactory.LoadFromFileAsync(path).Result; // Deadlock risk!

// âœ… Good - ConfigureAwait(false) in libraries
